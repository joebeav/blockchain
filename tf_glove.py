...
class GloVeModel():
    def __init__(self, embedding_size, context_size, pre_load_weights = None, pre_load_w2c = None, max_vocab_size=100000, min_occurrences=1,
        ...
        if not pre_load_weights is None:
            self.preload = True
            self.preload_embd = pre_load_weights
            self.preload_w2c = pre_load_w2c
    ...
    def __fit_to_corpus(self, corpus, vocab_size, min_occurrences, left_size, right_size):
        ...
        if self.preload:
            self.init_embeddings = np.random.uniform(1.0, -1.0, size = (self.vocab_size, self.embedding_size)).astype(dtype=np.float32)
            for word in self.words:
                try:
                    id_train = self.__word_to_id[word]
                    id_preload = self.preload_w2c[word]
                    self.init_embeddings[id_train] = self.preload_embd[id_preload]
                except:
                    pass
    ...
    def __build_graph(self):
        ...
        with self.__graph.as_default(), self.__graph.device(_device_for_node):
            ...
            if self.preload:
                focal_embeddings = tf.Variable(self.init_embeddings, name = "focal_embeddings")
                context_embeddings = tf.Variable(self.init_embeddings, name = "context_embeddings")
